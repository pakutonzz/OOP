...
class Subject:
    def __init__(self, subject_id, subject_name, subject_credit):
        self.__subject_id = subject_id
        self.__subject_name = subject_name
        self.__subject_credit = subject_credit
...
def enroll_to_subject(student, subject):
    if isinstance(student, Student) and isinstance(subject, Subject) :
        if search_enrollment_subject_student(subject , student) : 
...
def assign_grade(student, subject, grade):
    check = search_enrollment_subject_student(subject, student)
    if check :
        if check.grade == None :
            check.grade = grade
            return "Done"
...
def get_teacher_teach(subject_search):
    for subject in subject_list :
        if subject.subject_id == subject_search.subject_id :
            return subject.subject_teacher
...
def get_student_GPS(student):
    ...
    #GPS = subject_credit * grade / sum(subject_credit)
    sum_credit = 0
    sum_grade = 0

    for subject in subject_list :
        if subject.subject_id in Student_record :
            sum_grade += subject.subject_credit * grade_to_count(Student_record[subject.subject_id][1])
            sum_credit += subject.subject_credit
