...
class Account:
    ...
    def __irshift__(self, payload) :
        ...
        self.__transaction.append(Transaction("transfer_to", amount, self.__amount, target_account))
        target_account.__transaction.append(Transaction("recieve_money", amount, target_account.__amount, self))
    ...
    def __iter__(self) : 
        for transaction in self.__transaction :
            if transaction.transaction_type == "transfer_to" :
                yield f'{transaction.transaction_type} {transaction.target_account.user.name}  amount: {transaction.amount}  total: {transaction.total}'
            else :
                yield f'{transaction.transaction_type}  amount: {transaction.amount}  total: {transaction.total}'
